...
{
 $id=$_SESSION['id'];
 $vq=mysqli_query($con,"select * from tblvendor where pld_email='$id'");
 $vr=mysqli_fetch_array($vq);
 $vrid=$vr['pldvendor_id'];
}
...
{
$query=mysqli_query($con,"select * from tblvendor where pld_email='$id'");
if(mysqli_num_rows($query))
{   if(!file_exists("image/supplier/".$id."/productimages"))
	{
		$dir=mkdir("image/supplier/".$id."/productimages");
	}
	$row=mysqli_fetch_array($query);
    $v_id=$row['pldvendor_id'];
}
else
{
...
{   if(isset($_SESSION['id']))
	...
	if(mysqli_query($con,"insert into tbproduct(pldvendor_id,product,cost,description,paymentmode,pldimage) values
	
	('$v_id','$product_name','$cost','$description','$paymentmode','$img_name')"))
	{
		move_uploaded_file($_FILES['product_pic']['tmp_name'],"image/supplier/$id/productimages/".$_FILES['product_pic']['name']);
	}
	else{
		echo "failed";
	}
...
if(isset($upd_account))
				{
					if(mysqlI_query($con,"update tblvendor set pld_name='$fn',pld_email='$emm',pld_address='$add',pld_mob='$mob',pld_phone='$phn', pld_password='$pwsd' where pld_email='$id'"))
				   {
			  ...
			  if(isset($upd_logo))
			  {
				  if(isset($_SESSION['id']))
				  {
				  $log_img=mysqli_query($con,"select * from tblvendor where pld_email='$id'");
                  $log_img_row=mysqli_fetch_array($log_img);
				  $old_logo=$log_img_row['pld_logo'];
				  $new_img_name=$_FILES['logo_pic']['name'];
				  
				  if(mysqli_query($con,"update tblvendor set pld_logo='$new_img_name' where pld_email='$id'"))
				  {
					  unlink("image/supplier/$id/$old_logo");
					  move_uploaded_file($_FILES['logo_pic']['tmp_name'],"image/supplier/$id/".$_FILES['logo_pic']['name']);
...
<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
	...
	{
	?>
	<a class="navbar-brand" style="color:black; text-decoration:none;"><i class="far fa-user"><?php if(isset($id)) { echo $vr['pld_name']; }?></i></a>
	<?php
	}
	?>
    ...
    <div class="collapse navbar-collapse" id="navbarResponsive">
      ...
      <ul class="navbar-nav ml-auto">
		...
		<li class="nav-item">
          <a class="nav-link" href="login/index.php">User Login</a>
        </li>
		<li class="nav-item">
		  <form method="post">
          <?php
	   ...
	   <div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade show active" id="viewitem" role="tabpanel" aria-labelledby="home-tab">
			 ...
			 <th>Update item Details </th>
			   <?php
					  if($query=mysqli_query($con,"select tblvendor.pldvendor_id,tblvendor.pld_email,tbproduct.product_id,tbproduct.product,tbproduct.cost,tbproduct.paymentmode,tbproduct.description,tbproduct.pldimage,tblvendor.pld_name from tblvendor inner join tbproduct on tblvendor.pldvendor_id=tbproduct.pldvendor_id where tblvendor.pld_email='$id'"))
					  {
						  if(mysqli_num_rows($query))
						  {
				...
				<td><img src="<?php echo 'image/supplier/'.$id.'/productimages/'.$row['pldimage'];?>" height="100px" width="150px"></td>
				<td style="width:150px;"><?php  echo $row['product']."<br>";?></td>
				...
				<a href="product.php?product_id=<?php echo $row['product_id'];?>"><button type="button" class="btn btn-warning">Delete </button></a>
...
<!--tab 1 ends-->	   
			         ...
			         <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
	                                 ...
	                                 <div class="form-group"><!--description-->
                                            ...
                                            <input type="text" class="form-control" id="description" value="<?php if(isset($description)) { echo $description;}?>" placeholder="Enter description" name="description" required>
                                    </div>
			...
			<!--tab 2 ends-->
			 ...
			 <div class="tab-pane fade" id="accountsettings" role="tabpanel" aria-labelledby="accountsettings-tab">
				...
				<?php
			    $upd_info=mysqli_query($con,"select * from tblvendor where pld_email='$id'");
				$upd_info_row=mysqlI_fetch_array($upd_info);
				 $nm=$upd_info_row['pld_name'];
				 $emm=$upd_info_row['pld_email'];
				 $log=$upd_info_row['pld_logo'];
				$ad=$upd_info_row['pld_address'];
				$mb=$upd_info_row['pld_mob'];
				$phn=$upd_info_row['pld_phone'];
				$psd=$upd_info_row['pld_password'];
				...
				?>
					...
					<div class="form-group">
                      ...
                      <input type="text" id="mobile" pattern="[0-9]{10}" value="<?php if(isset($mb)){ echo $mb;}?>" class="form-control" name="mob" required/>
                    </div>
					<div class="form-group">
                      <label for="mobile">Phone</label>
                      <input type="text" id="phone" pattern="[0-9]{10}" value="<?php if(isset($phn)){ echo $phn;}?>" class="form-control" name="phn" required/>
                    </div>
                   <div class="form-group">
			...
			<div class="tab-pane fade" id="logo" role="tabpanel" aria-labelledby="logo-tab">
			 ...
			 <div class="tab-pane fade " id="status" role="tabpanel" aria-labelledby="status-tab">
				...
				<th>Update Status</th>
				<?php
				$orderquery=mysqli_query($con,"select * from tblorder where pldvendor_id='$vrid'");
				if(mysqli_num_rows($orderquery))
				{
					while($orderrow=mysqli_fetch_array($orderquery))
					{
						$stat=$orderrow['pldstatus'];
						?>
						<tr>
						<td><?php echo $orderrow['pld_order_id']; ?></td>
						<td><?php echo $orderrow['pld_email_id']; ?></td>
						<td><?php echo $orderrow['pld_product_id']; ?></td>
						<?php
			   if($stat=="cancelled" || $stat=="Out Of Stock")
			   {
			   ?>
			   <td><i style="color:orange;" class="fas fa-exclamation-triangle"></i>&nbsp;<span style="color:red;"><?php echo $orderrow['pldstatus']; ?></span></td>
			   <?php
			   }
			   else
			   ...
			   {
			   ?>
			   <td><span style="color:green;"><?php echo $orderrow['pldstatus']; ?></span></td>
			   <?php
			   }
			   ?>
						<form method="post">
						<td><a href="changestatus.php?order_id=<?php echo $orderrow['pld_order_id']; ?>"><button type="button" name="changestatus">Update Status</button></a></td>
						</form>
						<tr>
						<?php
					}

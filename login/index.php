...
if (isset($login)) {
	$query = mysqli_query($con, "select * from tblcustomer where pld_email='$email' && password='$password'");
	if ($row = mysqli_fetch_array($query)) {
		$customer_email = $row['pld_email'];
...
if (isset($register)) {
	$query = mysqli_query($con, "select * from tblcustomer where pld_email='$email'");
	$row = mysqli_num_rows($query);
	if ($row) {
		$ermsg2 = "Email alredy registered with us";
	} else {
		if (mysqli_query($con, "insert into tblcustomer (pld_name,pld_email,password,pld_mobile) values('$name','$email','$password','$mobile')")) {
	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light fixed-top">
		...
		<div class="collapse navbar-collapse" id="navbarResponsive">
			<ul class="navbar-nav ml-auto">
				...
				<li class="nav-item">
					<a class="nav-link" href="../vendor_login.php">Vendor Login</a>
				</li>
			</ul>
		</div>
	</nav>
	<br><br><br>
	<div class="middle" style="position:relative; padding:40px; border:1px solid #ED2553; left:40%; top:25%; width:400px;">
		...
		<div class="tab-content" id="myTabContent">
			...
			<div class="tab-pane fade show active" id="login" role="tabpanel" aria-labelledby="home-tab">
				<div class="footer" style="color:red;"><?php if (isset($loginmsg)) {
															echo $loginmsg;
														} ?></div>
				<form method="post">
					...
					<button type="submit" name="login" style="background:#ED2553; border:1px solid #ED2553;" class="btn btn-primary">Sign In</button>
			...
			<div class="tab-pane fade" id="signup" role="tabpanel" aria-labelledby="profile-tab">
				<form method="post">
					...
					<div class="form-group">
						...
						<input type="tel" id="mobile" class="form-control" name="mobile" pattern="[0-9]{10}" placeholder="enter mobile number" required>
					</div>
